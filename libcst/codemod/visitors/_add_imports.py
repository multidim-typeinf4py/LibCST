...
class AddHintableImportsVisitor(ContextAwareTransformer):
    """
    Ensures that given imports exist in a module. Given a
    :class:`~libcst.codemod.CodemodContext` and a sequence of tuples specifying
    a module to import from as a string. Optionally an object to import from
    ...
    It will modify existing imports as necessary if the module in question is
    already being imported from.
    Additioanally, an argument`for_type_hint=True` may be passed to generate
    code that avoids circular dependencies at runtime
    ...
    For example::
        ...
        AddHintableImportsVisitor.add_needed_import(self.context, "typing", "Optional")
    ...
    As another example::
        ...
        AddHintableImportsVisitor.add_needed_import(self.context, "typing")
    ...
    the automatic transform feature of :class:`~libcst.codemod.CodemodCommand`
    and schedule an import to be added by calling
    :meth:`~libcst.codemod.visitors.AddHintableImportsVisitor.add_needed_import`
    """

    CONTEXT_KEY = "AddHintableImportsVisitor"

    @staticmethod
    def _get_imports_from_context(
        context: CodemodContext,
    ) -> tuple[list[ImportItem], list[ImportItem]]:
        imports = context.scratch.get(AddHintableImportsVisitor.CONTEXT_KEY, ([], []))
    ...
    def add_needed_import(
        ...
        if module == "__future__" and obj is None:
            raise Exception("Cannot import __future__ directly!")
        imports, hinting_imports = AddHintableImportsVisitor._get_imports_from_context(context)

        item = ImportItem(module, obj, asname, relative)
        if not for_type_hint:
            imports.append(item)
        else:
            hinting_imports.append(item)
        context.scratch[AddHintableImportsVisitor.CONTEXT_KEY] = (imports, hinting_imports)
    ...
    def __init__(
        ...
        ctx_imports, hinting_imports = AddHintableImportsVisitor._get_imports_from_context(
            context
        )

        nonhinted_imps: List[ImportItem] = [
            *ctx_imports,
            *imports,
        ]

        imps: list[tuple[ImportItem, bool]] = list(
            itertools.chain(
                zip(nonhinted_imps, itertools.repeat(False)),
                zip(hinting_imports, itertools.repeat(True)),
            )
        )
        ...
        # Resolve relative imports if we have a module name
        imps = [
            (imp.resolve_relative(self.context.full_package_name), for_hinting)
            for imp, for_hinting in imps
        ]

        # List of modules we need to ensure are imported
        self.module_imports: dict[str, bool] = {
            imp.module: for_hinting
            for imp, for_hinting in imps
            if imp.obj_name is None and imp.alias is None
        }

        # List of modules we need to check for object imports on
        from_imports: dict[str, bool] = {
            imp.module: for_hinting
            for imp, for_hinting in imps
            if imp.obj_name is not None and imp.alias is None
        }
        # Mapping of modules we're adding to the object they should import
        self.module_mapping: Dict[str, Set[tuple[str, bool]]] = {
            module: {
                (imp.obj_name, for_hinting)
                for imp, for_hinting in imps
                if imp.module == module
                and imp.obj_name is not None
                and imp.alias is None
        ...
        # List of aliased modules we need to ensure are imported
        self.module_aliases: Dict[str, tuple[str, bool]] = {
            imp.module: (imp.alias, for_hinting)
            for imp, for_hinting in imps
            if imp.obj_name is None and imp.alias is not None
        }
        # List of modules we need to check for object imports on
        from_imports_aliases: dict[str, bool] = {
            imp.module: for_hinting
            for imp, for_hinting in imps
            if imp.obj_name is not None and imp.alias is not None
        }
        # Mapping of modules we're adding to the object with alias they should import
        self.alias_mapping: Dict[str, List[Tuple[str, str, bool]]] = {
            module: [
                (imp.obj_name, imp.alias, for_hinting)
                for imp, for_hinting in imps
                if imp.module == module
                and imp.obj_name is not None
                and imp.alias is not None
    ...
    def visit_Module(self, node: libcst.Module) -> None:
        ...
        self.module_imports = {
            imp: for_hinting
            for imp, for_hinting in self.module_imports
            if imp not in gatherer.module_imports
        }
        for module, alias in gatherer.module_aliases.items():
            if (
                module in self.module_aliases
                and self.module_aliases[module][0] == alias
            ):
                del self.module_aliases[module]
        for module, aliases in gatherer.alias_mapping.items():
            for (obj, alias) in aliases:
                hinted_import = (obj, alias, True)
                std_import = (obj, alias, False)
                if (
                    module in self.alias_mapping
                    and hinted_import in self.alias_mapping[module]
                ):
                    self.alias_mapping[module].remove(hinted_import)
                    if len(self.alias_mapping[module]) == 0:
                        del self.alias_mapping[module]

                if (
                    module in self.alias_mapping
                    and std_import in self.alias_mapping[module]
                ):
                    self.alias_mapping[module].remove(std_import)
                    if len(self.alias_mapping[module]) == 0:
                        del self.alias_mapping[module]
    ...
    def leave_ImportFrom(
        ...
        return updated_node.with_changes(
            names=[
                *(
                    libcst.ImportAlias(name=libcst.Name(imp))
                    for imp, for_hinting in sorted(imports_to_add)
                    if not for_hinting
                ),
                *(
                    libcst.ImportAlias(
    ...
    def leave_Module(
        ...
        # Mapping of modules we're adding to the object with as plain imports and without alias they should import
        module_and_alias_mapping = defaultdict(list)

        # Mapping of modules we're adding to the object as hinting imports and without alias they should import
        hinting_imports = defaultdict(list)

        for module, (*aliases, for_hinting) in self.alias_mapping.items():
            if not for_hinting:
                module_and_alias_mapping[module].extend(aliases)
            else:
                hinting_imports[module].extend(aliases)

        for module, imports in self.module_mapping.items():
            for (imp, for_hinting) in imports:
                if not for_hinting:
                    module_and_alias_mapping[module].append((imp, None))
                else:
                    hinting_imports[module].append((imp, None))
        module_and_alias_mapping = {
            module: sorted(aliases)
            for module, aliases in module_and_alias_mapping.items()
        }
        ...
        hinting_imports = {
            module: sorted(aliases) for module, aliases in hinting_imports.items()
        }
        # Now, add all of the imports we need!

        if hinting_imports:
            typing_import = libcst.parse_statement("from typing import TYPE_CHECKING")
            typing_check = "if TYPE_CHECKING:"
            typing_imports = [
                f"from {module} import "
                + ", ".join(
                    [
                ...
                for module, aliases in hinting_imports.items()
                if module != "__future__"
            ]

            type_hint_imports = libcst.parse_statement(
                f"{typing_check}\n"
                + "\n".join(textwrap.indent(imp, prefix="\t") for imp in typing_imports)
            )

            type_hints = (typing_import, type_hint_imports)
        else:
            type_hints = tuple()
